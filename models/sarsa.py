...
from .utils import SARSA_Parameters, _ACTIONS, _FLAP, _NO_FLAP, _MIN_V, _MAX_V, _MAX_X, _MAX_Y

#SARSA aggiorna q-valori con valori stato prossimo e azione dello stato attuale
class SARSA:
    def __init__(self):
        self.eps_start = SARSA_Parameters["epsilon_start"]
        self.eps_decay = SARSA_Parameters["epsilon_decay"]
        self.eps_end = SARSA_Parameters["epsilon_end"]
        self.alpha = SARSA_Parameters["alpha"]
        self.alpha_end = SARSA_Parameters["alpha_end"]
        self.alpha_decay = SARSA_Parameters["alpha_decay"]
        self.gamma = SARSA_Parameters["gamma"]
        self.partitions = SARSA_Parameters["partitions"]
        self.steps_done = SARSA_Parameters["steps_done"]
    ...
    def observe(self, s1, a, r, s2, end):
        ...
        future_action = self.training_policy(s2)
        future_reward = self.Q_table[(s2, future_action)] if not end else 0
